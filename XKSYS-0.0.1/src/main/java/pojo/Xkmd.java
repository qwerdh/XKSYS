package pojo;

import java.util.Date;

public class Xkmd {
    private Integer idn;

    private String xq;

    private String xh;

    private String bh;

    private String bj;

    private String xm;

    private String kcdm;

    private String kcmc;

    private String lbdh;

    private Integer zc;

    private Integer jc1;

    private Integer jc2;

    private Boolean tx;

    private Boolean jcqk;

    private String jcdm;

    private Integer kcxh;

    private Integer kcidn;

    private Integer czzs;

    private Date czrq;

    private Integer xkzs;

    private Date xkrq;

    public Integer getIdn() {
        return idn;
    }

    public void setIdn(Integer idn) {
        this.idn = idn;
    }

    public String getXq() {
        return xq;
    }

    public void setXq(String xq) {
        this.xq = xq == null ? null : xq.trim();
    }

    public String getXh() {
        return xh;
    }

    public void setXh(String xh) {
        this.xh = xh == null ? null : xh.trim();
    }

    public String getBh() {
        return bh;
    }

    public void setBh(String bh) {
        this.bh = bh == null ? null : bh.trim();
    }

    public String getBj() {
        return bj;
    }

    public void setBj(String bj) {
        this.bj = bj == null ? null : bj.trim();
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getKcdm() {
        return kcdm;
    }

    public void setKcdm(String kcdm) {
        this.kcdm = kcdm == null ? null : kcdm.trim();
    }

    public String getKcmc() {
        return kcmc;
    }

    public void setKcmc(String kcmc) {
        this.kcmc = kcmc == null ? null : kcmc.trim();
    }

    public String getLbdh() {
        return lbdh;
    }

    public void setLbdh(String lbdh) {
        this.lbdh = lbdh == null ? null : lbdh.trim();
    }

    public Integer getZc() {
        return zc;
    }

    public void setZc(Integer zc) {
        this.zc = zc;
    }

    public Integer getJc1() {
        return jc1;
    }

    public void setJc1(Integer jc1) {
        this.jc1 = jc1;
    }

    public Integer getJc2() {
        return jc2;
    }

    public void setJc2(Integer jc2) {
        this.jc2 = jc2;
    }

    public Boolean getTx() {
        return tx;
    }

    public void setTx(Boolean tx) {
        this.tx = tx;
    }

    public Boolean getJcqk() {
        return jcqk;
    }

    public void setJcqk(Boolean jcqk) {
        this.jcqk = jcqk;
    }

    public String getJcdm() {
        return jcdm;
    }

    public void setJcdm(String jcdm) {
        this.jcdm = jcdm == null ? null : jcdm.trim();
    }

    public Integer getKcxh() {
        return kcxh;
    }

    public void setKcxh(Integer kcxh) {
        this.kcxh = kcxh;
    }

    public Integer getKcidn() {
        return kcidn;
    }

    public void setKcidn(Integer kcidn) {
        this.kcidn = kcidn;
    }

    public Integer getCzzs() {
        return czzs;
    }

    public void setCzzs(Integer czzs) {
        this.czzs = czzs;
    }

    public Date getCzrq() {
        return czrq;
    }

    public void setCzrq(Date czrq) {
        this.czrq = czrq;
    }

    public Integer getXkzs() {
        return xkzs;
    }

    public void setXkzs(Integer xkzs) {
        this.xkzs = xkzs;
    }

    public Date getXkrq() {
        return xkrq;
    }

    public void setXkrq(Date xkrq) {
        this.xkrq = xkrq;
    }
}