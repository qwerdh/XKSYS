package pojo;

import java.io.Serializable;
import java.util.Date;

public class XsVNew implements Serializable{
    private String xh;

    private String bh;

    private String xm;

    private String xb;

    private String mzm;

    private Date csny;

    private String jg;

    private String sym;

    private String xszh;

    private String xhh;

    private Double cet4;

    private Date pcet4;

    private Double cet6;

    private Date pcet6;

    private String jsjdj1;

    private Date pjsjdj1;

    private String jsjdj2;

    private Date pjsjdj2;

    private Integer ym;

    private Integer hh;

    private String th;

    private String kl;

    private Integer xjqkdm;

    private String bjy;

    private String zsbh;

    private String xwzsbh;

    private Date bynf;

    private Integer idn;

    private Boolean scbj;

    private String bmmc;

    private String xbmc;

    private String xjqk;

    private String bj;

    private String xz;

    private String xbdm;

    private String zkzh;

    private String sfzh;

    private Date rxsj;

    private String ybh;

    private String zymc;

    private String yzbm;

    private String jtdz;

    private String jtdh;

    private String sf;

    private String zzmm;

    private String yxxmc;

    private String jzxm;

    private String gx;

    private Boolean lx;

    private String zxqk;

    private String ksh;

    private String smscode;

    private String fxdm;

    private String pkbmmc;

    private String syxz;

    private String xwlx;

    private String xwm;

    private String zydm;

    private String sylb;

    private String lb;

    private String sylbdm;

    private String gjdm;

    private String xwgjdm;

    private String jkzk;

    private String tel;

    private Date bysj;

    private String zydmBj;

    private Date bysjBj;

    private String xmEng;

    private Integer rxnf;

    private Double nj;

    private String sbZydm;

    private String sbZymc;

    private String sbXm;

    private String sbXz;

    private String sbCc;

    private String sbRxrq;

    private String sbByrq;

    private String zxh;

    private String kslb;

    private String code1;

    private String openid;

    private String bysm;

    public String getXh() {
        return xh;
    }

    public void setXh(String xh) {
        this.xh = xh == null ? null : xh.trim();
    }

    public String getBh() {
        return bh;
    }

    public void setBh(String bh) {
        this.bh = bh == null ? null : bh.trim();
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getMzm() {
        return mzm;
    }

    public void setMzm(String mzm) {
        this.mzm = mzm == null ? null : mzm.trim();
    }

    public Date getCsny() {
        return csny;
    }

    public void setCsny(Date csny) {
        this.csny = csny;
    }

    public String getJg() {
        return jg;
    }

    public void setJg(String jg) {
        this.jg = jg == null ? null : jg.trim();
    }

    public String getSym() {
        return sym;
    }

    public void setSym(String sym) {
        this.sym = sym == null ? null : sym.trim();
    }

    public String getXszh() {
        return xszh;
    }

    public void setXszh(String xszh) {
        this.xszh = xszh == null ? null : xszh.trim();
    }

    public String getXhh() {
        return xhh;
    }

    public void setXhh(String xhh) {
        this.xhh = xhh == null ? null : xhh.trim();
    }

    public Double getCet4() {
        return cet4;
    }

    public void setCet4(Double cet4) {
        this.cet4 = cet4;
    }

    public Date getPcet4() {
        return pcet4;
    }

    public void setPcet4(Date pcet4) {
        this.pcet4 = pcet4;
    }

    public Double getCet6() {
        return cet6;
    }

    public void setCet6(Double cet6) {
        this.cet6 = cet6;
    }

    public Date getPcet6() {
        return pcet6;
    }

    public void setPcet6(Date pcet6) {
        this.pcet6 = pcet6;
    }

    public String getJsjdj1() {
        return jsjdj1;
    }

    public void setJsjdj1(String jsjdj1) {
        this.jsjdj1 = jsjdj1 == null ? null : jsjdj1.trim();
    }

    public Date getPjsjdj1() {
        return pjsjdj1;
    }

    public void setPjsjdj1(Date pjsjdj1) {
        this.pjsjdj1 = pjsjdj1;
    }

    public String getJsjdj2() {
        return jsjdj2;
    }

    public void setJsjdj2(String jsjdj2) {
        this.jsjdj2 = jsjdj2 == null ? null : jsjdj2.trim();
    }

    public Date getPjsjdj2() {
        return pjsjdj2;
    }

    public void setPjsjdj2(Date pjsjdj2) {
        this.pjsjdj2 = pjsjdj2;
    }

    public Integer getYm() {
        return ym;
    }

    public void setYm(Integer ym) {
        this.ym = ym;
    }

    public Integer getHh() {
        return hh;
    }

    public void setHh(Integer hh) {
        this.hh = hh;
    }

    public String getTh() {
        return th;
    }

    public void setTh(String th) {
        this.th = th == null ? null : th.trim();
    }

    public String getKl() {
        return kl;
    }

    public void setKl(String kl) {
        this.kl = kl == null ? null : kl.trim();
    }

    public Integer getXjqkdm() {
        return xjqkdm;
    }

    public void setXjqkdm(Integer xjqkdm) {
        this.xjqkdm = xjqkdm;
    }

    public String getBjy() {
        return bjy;
    }

    public void setBjy(String bjy) {
        this.bjy = bjy == null ? null : bjy.trim();
    }

    public String getZsbh() {
        return zsbh;
    }

    public void setZsbh(String zsbh) {
        this.zsbh = zsbh == null ? null : zsbh.trim();
    }

    public String getXwzsbh() {
        return xwzsbh;
    }

    public void setXwzsbh(String xwzsbh) {
        this.xwzsbh = xwzsbh == null ? null : xwzsbh.trim();
    }

    public Date getBynf() {
        return bynf;
    }

    public void setBynf(Date bynf) {
        this.bynf = bynf;
    }

    public Integer getIdn() {
        return idn;
    }

    public void setIdn(Integer idn) {
        this.idn = idn;
    }

    public Boolean getScbj() {
        return scbj;
    }

    public void setScbj(Boolean scbj) {
        this.scbj = scbj;
    }

    public String getBmmc() {
        return bmmc;
    }

    public void setBmmc(String bmmc) {
        this.bmmc = bmmc == null ? null : bmmc.trim();
    }

    public String getXbmc() {
        return xbmc;
    }

    public void setXbmc(String xbmc) {
        this.xbmc = xbmc == null ? null : xbmc.trim();
    }

    public String getXjqk() {
        return xjqk;
    }

    public void setXjqk(String xjqk) {
        this.xjqk = xjqk == null ? null : xjqk.trim();
    }

    public String getBj() {
        return bj;
    }

    public void setBj(String bj) {
        this.bj = bj == null ? null : bj.trim();
    }

    public String getXz() {
        return xz;
    }

    public void setXz(String xz) {
        this.xz = xz == null ? null : xz.trim();
    }

    public String getXbdm() {
        return xbdm;
    }

    public void setXbdm(String xbdm) {
        this.xbdm = xbdm == null ? null : xbdm.trim();
    }

    public String getZkzh() {
        return zkzh;
    }

    public void setZkzh(String zkzh) {
        this.zkzh = zkzh == null ? null : zkzh.trim();
    }

    public String getSfzh() {
        return sfzh;
    }

    public void setSfzh(String sfzh) {
        this.sfzh = sfzh == null ? null : sfzh.trim();
    }

    public Date getRxsj() {
        return rxsj;
    }

    public void setRxsj(Date rxsj) {
        this.rxsj = rxsj;
    }

    public String getYbh() {
        return ybh;
    }

    public void setYbh(String ybh) {
        this.ybh = ybh == null ? null : ybh.trim();
    }

    public String getZymc() {
        return zymc;
    }

    public void setZymc(String zymc) {
        this.zymc = zymc == null ? null : zymc.trim();
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm == null ? null : yzbm.trim();
    }

    public String getJtdz() {
        return jtdz;
    }

    public void setJtdz(String jtdz) {
        this.jtdz = jtdz == null ? null : jtdz.trim();
    }

    public String getJtdh() {
        return jtdh;
    }

    public void setJtdh(String jtdh) {
        this.jtdh = jtdh == null ? null : jtdh.trim();
    }

    public String getSf() {
        return sf;
    }

    public void setSf(String sf) {
        this.sf = sf == null ? null : sf.trim();
    }

    public String getZzmm() {
        return zzmm;
    }

    public void setZzmm(String zzmm) {
        this.zzmm = zzmm == null ? null : zzmm.trim();
    }

    public String getYxxmc() {
        return yxxmc;
    }

    public void setYxxmc(String yxxmc) {
        this.yxxmc = yxxmc == null ? null : yxxmc.trim();
    }

    public String getJzxm() {
        return jzxm;
    }

    public void setJzxm(String jzxm) {
        this.jzxm = jzxm == null ? null : jzxm.trim();
    }

    public String getGx() {
        return gx;
    }

    public void setGx(String gx) {
        this.gx = gx == null ? null : gx.trim();
    }

    public Boolean getLx() {
        return lx;
    }

    public void setLx(Boolean lx) {
        this.lx = lx;
    }

    public String getZxqk() {
        return zxqk;
    }

    public void setZxqk(String zxqk) {
        this.zxqk = zxqk == null ? null : zxqk.trim();
    }

    public String getKsh() {
        return ksh;
    }

    public void setKsh(String ksh) {
        this.ksh = ksh == null ? null : ksh.trim();
    }

    public String getSmscode() {
        return smscode;
    }

    public void setSmscode(String smscode) {
        this.smscode = smscode == null ? null : smscode.trim();
    }

    public String getFxdm() {
        return fxdm;
    }

    public void setFxdm(String fxdm) {
        this.fxdm = fxdm == null ? null : fxdm.trim();
    }

    public String getPkbmmc() {
        return pkbmmc;
    }

    public void setPkbmmc(String pkbmmc) {
        this.pkbmmc = pkbmmc == null ? null : pkbmmc.trim();
    }

    public String getSyxz() {
        return syxz;
    }

    public void setSyxz(String syxz) {
        this.syxz = syxz == null ? null : syxz.trim();
    }

    public String getXwlx() {
        return xwlx;
    }

    public void setXwlx(String xwlx) {
        this.xwlx = xwlx == null ? null : xwlx.trim();
    }

    public String getXwm() {
        return xwm;
    }

    public void setXwm(String xwm) {
        this.xwm = xwm == null ? null : xwm.trim();
    }

    public String getZydm() {
        return zydm;
    }

    public void setZydm(String zydm) {
        this.zydm = zydm == null ? null : zydm.trim();
    }

    public String getSylb() {
        return sylb;
    }

    public void setSylb(String sylb) {
        this.sylb = sylb == null ? null : sylb.trim();
    }

    public String getLb() {
        return lb;
    }

    public void setLb(String lb) {
        this.lb = lb == null ? null : lb.trim();
    }

    public String getSylbdm() {
        return sylbdm;
    }

    public void setSylbdm(String sylbdm) {
        this.sylbdm = sylbdm == null ? null : sylbdm.trim();
    }

    public String getGjdm() {
        return gjdm;
    }

    public void setGjdm(String gjdm) {
        this.gjdm = gjdm == null ? null : gjdm.trim();
    }

    public String getXwgjdm() {
        return xwgjdm;
    }

    public void setXwgjdm(String xwgjdm) {
        this.xwgjdm = xwgjdm == null ? null : xwgjdm.trim();
    }

    public String getJkzk() {
        return jkzk;
    }

    public void setJkzk(String jkzk) {
        this.jkzk = jkzk == null ? null : jkzk.trim();
    }

    public String getTel() {
        return tel;
    }

    public void setTel(String tel) {
        this.tel = tel == null ? null : tel.trim();
    }

    public Date getBysj() {
        return bysj;
    }

    public void setBysj(Date bysj) {
        this.bysj = bysj;
    }

    public String getZydmBj() {
        return zydmBj;
    }

    public void setZydmBj(String zydmBj) {
        this.zydmBj = zydmBj == null ? null : zydmBj.trim();
    }

    public Date getBysjBj() {
        return bysjBj;
    }

    public void setBysjBj(Date bysjBj) {
        this.bysjBj = bysjBj;
    }

    public String getXmEng() {
        return xmEng;
    }

    public void setXmEng(String xmEng) {
        this.xmEng = xmEng == null ? null : xmEng.trim();
    }

    public Integer getRxnf() {
        return rxnf;
    }

    public void setRxnf(Integer rxnf) {
        this.rxnf = rxnf;
    }

    public Double getNj() {
        return nj;
    }

    public void setNj(Double nj) {
        this.nj = nj;
    }

    public String getSbZydm() {
        return sbZydm;
    }

    public void setSbZydm(String sbZydm) {
        this.sbZydm = sbZydm == null ? null : sbZydm.trim();
    }

    public String getSbZymc() {
        return sbZymc;
    }

    public void setSbZymc(String sbZymc) {
        this.sbZymc = sbZymc == null ? null : sbZymc.trim();
    }

    public String getSbXm() {
        return sbXm;
    }

    public void setSbXm(String sbXm) {
        this.sbXm = sbXm == null ? null : sbXm.trim();
    }

    public String getSbXz() {
        return sbXz;
    }

    public void setSbXz(String sbXz) {
        this.sbXz = sbXz == null ? null : sbXz.trim();
    }

    public String getSbCc() {
        return sbCc;
    }

    public void setSbCc(String sbCc) {
        this.sbCc = sbCc == null ? null : sbCc.trim();
    }

    public String getSbRxrq() {
        return sbRxrq;
    }

    public void setSbRxrq(String sbRxrq) {
        this.sbRxrq = sbRxrq == null ? null : sbRxrq.trim();
    }

    public String getSbByrq() {
        return sbByrq;
    }

    public void setSbByrq(String sbByrq) {
        this.sbByrq = sbByrq == null ? null : sbByrq.trim();
    }

    public String getZxh() {
        return zxh;
    }

    public void setZxh(String zxh) {
        this.zxh = zxh == null ? null : zxh.trim();
    }

    public String getKslb() {
        return kslb;
    }

    public void setKslb(String kslb) {
        this.kslb = kslb == null ? null : kslb.trim();
    }

    public String getCode1() {
        return code1;
    }

    public void setCode1(String code1) {
        this.code1 = code1 == null ? null : code1.trim();
    }

    public String getOpenid() {
        return openid;
    }

    public void setOpenid(String openid) {
        this.openid = openid == null ? null : openid.trim();
    }

    public String getBysm() {
        return bysm;
    }

    public void setBysm(String bysm) {
        this.bysm = bysm == null ? null : bysm.trim();
    }
}